...
export function catalogueProfile(model:string):PrintProfile|undefined{
 ...
 if(p.kind==='gin'||p.kind==='flute'){
  ...
  const curve=new THREE.CubicBezierCurve(new THREE.Vector2(.027,bowlBase),new THREE.Vector2(flute?top*.5:.027,h*(flute?.52:.56)),new THREE.Vector2(top,h*(flute?.69:.55)),new THREE.Vector2(top,h*(flute?.87:.82)));
 ...
 return {points,printMin:.09,handle:p.kind==='mug',lid:p.kind==='bucks'?'bucks':p.kind==='bottle'?'bottle':undefined,fullHeight:h,lidRadius:p.kind==='bucks'?3.9*scale:undefined,band:{bottom:.12,top:.91},ridges:p.kind==='disposable'};
}
...
export function addCatalogueDetails(scene:THREE.Scene,profile:PrintProfile,mat:THREE.MeshPhysicalMaterial){
 ...
 if(profile.lid){
  ...
  const dh=(profile.fullHeight??y+.12)-y,capRadius=profile.lidRadius??r*1.03;
  if(profile.lid==='bucks'){
   add(new THREE.CylinderGeometry(capRadius,capRadius,dh*.28,96),cap,0,y+dh*.14);
   add(new THREE.CylinderGeometry(capRadius*.86,capRadius*.96,dh*.72,96),cap,0,y+dh*.64);
