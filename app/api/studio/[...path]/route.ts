...
function validateProducts(value:unknown):Product[]{
 ...
 const seen=new Set();return (value as Product[]).map(p=>{if(!p||typeof p.id!=="string"||!/^[-a-zA-Z0-9]{1,80}$/.test(p.id)||seen.has(p.id))fail(400,"Produto inválido.");seen.add(p.id);if(!(PRINTS as readonly string[]).includes(p.print)||!(MODELS as readonly string[]).includes(p.model)||!colorsFor(p.print,p.model).includes(p.color))fail(400,"Configuração de produto inválida.");if(p.art && (typeof p.art.id!=="string"||!/^[-a-zA-Z0-9]{1,80}$/.test(p.art.id)))fail(400,"Arte inválida.");if(p.artMode!==undefined&&p.artMode!=="logo"&&p.artMode!=="template")fail(400,"Aplicação da arte inválida.");return {...(p.artMode?{artMode:p.artMode}:{}),id:p.id,print:p.print,model:p.model,color:p.color,art:p.art?{id:p.art.id,name:"",type:"",size:0}:null};});
}
