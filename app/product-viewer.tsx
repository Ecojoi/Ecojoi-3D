...
export default function ProductViewer({product,token}:{product:Product;token?:string}){
 ...
 camera.position.set(0,height*.75,height*3.7);controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,height*.49,0);controls.enableDamping=true;controls.enablePan=false;controls.minDistance=height*1.8;controls.maxDistance=height*7;controls.maxPolarAngle=Math.PI*.88;controls.autoRotate=false;controls.autoRotateSpeed=.6;if(viewState.current?.model===product.model){camera.position.fromArray(viewState.current.position);controls.target.fromArray(viewState.current.target);}controls.addEventListener("change",()=>{viewState.current={model:product.model,position:camera.position.toArray(),target:controls!.target.toArray()};});controls.addEventListener("start",()=>{if(controls)controls.autoRotate=false;});reset.current=()=>{camera.position.set(0,height*.75,height*3.7);controls?.target.set(0,height*.49,0);if(controls)controls.autoRotate=false;};
 ...
 const resize=()=>{if(!renderer)return;const w=el.clientWidth||600,h=el.clientHeight||480;renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();};observer=new ResizeObserver(resize);observer.observe(el);resize();visibility=new IntersectionObserver(entries=>{visible=entries[0]?.isIntersecting??true;});visibility.observe(el);const tick=()=>{if(disposed)return;if(visible){controls?.update();renderer?.render(scene,camera);}frame=requestAnimationFrame(tick);};tick();
 ...
 return ()=>{disposed=true;cancelAnimationFrame(frame);observer?.disconnect();visibility?.disconnect();controls?.dispose();textures.forEach(t=>t.dispose());const materials=new Set<THREE.Material>();scene.traverse(o=>{if(o instanceof THREE.Mesh){o.geometry.dispose();(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>materials.add(m));}});materials.forEach(m=>m.dispose());environment?.dispose();renderer?.dispose();el.replaceChildren();};
